...
@section('title', 'Sửa công cụ học')

@section('content')
<div class="card shadow-sm">
    <div class="card-header bg-white border-bottom py-3">
        <h3 class="card-title mb-0 font-weight-bold text-dark">Sửa công cụ học: {{ $tool->name }}</h3>
        <a href="{{ route('admin.learning-tools.index') }}" class="btn btn-sm btn-outline-secondary"><i class="fas fa-arrow-left"></i> Danh sách</a>
    </div>
    <form action="{{ route('admin.learning-tools.update', $tool) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <input type="number" name="quantity" class="form-control @error('quantity') is-invalid @enderror" value="{{ old('quantity', $tool->quantity) }}" min="0" required>
                @error('quantity')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            <div class="form-group">
                ...
                <select name="center_id" id="center_id" class="form-control @error('center_id') is-invalid @enderror" required>
                    @foreach($centers as $c)
                        <option value="{{ $c->id }}" {{ old('center_id', $tool->center_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                    @endforeach
                </select>
                @error('center_id')
                    <span class="invalid-feedback">{{ $message }}</span>
                ...
                <select name="center_class_id" id="center_class_id" class="form-control @error('center_class_id') is-invalid @enderror">
                    ...
                    @foreach($centers as $c)
                        @foreach($c->classes as $cls)
                            <option value="{{ $cls->id }}" data-center="{{ $c->id }}" {{ old('center_class_id', $tool->center_class_id) == $cls->id ? 'selected' : '' }}>{{ $c->name }} – {{ $cls->name }}</option>
                        @endforeach
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="managed_by_user_id" class="form-control @error('managed_by_user_id') is-invalid @enderror">
                    ...
                    @foreach($managers as $u)
                        <option value="{{ $u->id }}" {{ old('managed_by_user_id', $tool->managed_by_user_id) == $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->role }})</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label>Ghi chú</label>
                <textarea name="note" class="form-control @error('note') is-invalid @enderror" rows="2">{{ old('note', $tool->note) }}</textarea>
                @error('note')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
